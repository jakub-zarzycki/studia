...
(* funkcja height zwraca wysokosc drzewa*)
let height = function
  | Node (_, _, _, _, h) -> h
  | Empty -> 0
...
(* Funkcja make tworzy drzewo majace poddrzewa l i r *)
(* oraz korzen przechowujacy przedzial k*)
let make l k r = Node (l, k, r, sum l k r, max (height l) (height r) + 1)

(* Funkcja bal wywaza drzewo tak, aby roznica wysokosci *)
(* lewego i prawego poddrzewa byla nie wieksza niz dwa *)
let bal l k r =
  let hl = height l in
  let hr = height r in
  if hl > hr + 2 then
...
(* Funkcja min_elt zwraca najmniejszy przedzial w zbiorze*)
let rec min_elt = function
  | Node (Empty, k, _, _, _) -> k
  | Node (l, _, _, _, _) -> min_elt l
  | Empty -> raise Not_found

(* Funkcja remove_min_elt zwraca zbior bez najmniejszego jego przedzialu*)
let rec remove_min_elt = function
  | Node (Empty, _, r, _, _) -> r
  | Node (l, k, r, _, _) -> bal (remove_min_elt l) k r
  | Empty -> invalid_arg "ISet.remove_min_elt"

(* Funkcja remove_one zwraca roznice przedzialow (fst, snd) \ (beg, endl) *)
(* przy zalozeniu ze nie wystapi przypadek fst < beg && endl < snd *)
let remove_one (fst, snd) (beg, endl) =
  if fst < beg && endl < snd then invalid_arg "Iset.remove_one"
  else if beg <= fst && snd <= endl then None
  else if beg > snd || endl < fst then Some (fst, snd)
  else if fst < beg then Some (fst, beg - 1)
  else Some (endl + 1, snd)     

(* Funkcja merge laczy dwa drzewa t1 i t2 *)
let merge t1 t2 =
  match t1, t2 with
  | Empty, _ -> t2
  | _, Empty -> t1
...
(* Funkcja join tworzy zrownowazone drzewo zawierajace drzewa l, r i przedzial v *)
let rec join l v r =
  match (l, r) with
  | (Empty, _) -> add_one v r
  | (_, Empty) -> add_one v l
...
let remove ((beg, endl) as x) set =
  let rec loop lb rb set =
    if lb && rb then Empty else match set with
    | Empty -> Empty
    | Node (l, ((fst, snd) as k), r, _, _) ->	
        if beg > fst && endl < snd then
          merge (add_one (fst, beg - 1) l) (add_one (endl + 1, snd) r)
	else
	  let p = remove_one k x in
	  let ll =
	    let rb = endl >= fst in
	    if beg < fst then loop lb rb l
	    else l in
          let rr =
	    let lb = beg <= snd in
	    if endl > snd then loop lb rb r
	    else r
	  in match p with
	    | None -> merge ll rr
	    | Some k -> join ll k rr
  in loop false false set		  
