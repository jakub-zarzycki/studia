 ...
 * disjoint intervals are well-ordered and intersecting intervals arehandled
 * via exceptions
 *)
  ...
  let rec loop x = function
    ...
    | Node (l, v, r, _) ->
      (try
        ...
        if c = 0 then 
          let lower, high = v in 
            (join cmp l (lower, (fst x)) Empty), 
            true, 
            (join cmp Empty ((fst x), high) r)
        else if c < 0 then
...
let mem (x : int) ({ cmp = cmp; set = set } : t) =
  let rec loop = function
    | Node (l, k, r, _) ->
        (try 
            let c = cmp (x, x) k in
        ...
        with
            (* if interval has non-empty intersection with any interval
               in the set, but is not equal then it cannot be a memeber *)
